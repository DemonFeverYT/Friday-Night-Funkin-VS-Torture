...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		loadingImage = new FlxSprite(0,0).loadGraphic(Paths.image('loadingButNotDone','torture'));
		loadingDone = new FlxSprite(0,0).loadGraphic(Paths.image('loadingButDone','torture'));
		...
		logoBl.frames = Paths.getSparrowAtlas('TrickyLogo','torture');
		...
		gfDance = new FlxSprite(FlxG.width * 0.16, FlxG.height * -0.22);
		gfDance.frames = Paths.getSparrowAtlas('DJ_Torture','torture');
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.55).loadGraphic(Paths.image('ThePalsV2','torture'));
		ngSpr.visible = false;
		...
		actualNG= new FlxSprite(0, FlxG.height * 0.55).loadGraphic(Paths.image('newgrounds_logo','torture'));
		...
		backupMen= new FlxSprite(0, FlxG.height * 0.55).loadGraphic(Paths.image('TheBackupMen','torture'));
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// music.play();
			FlxG.sound.playMusic(Paths.music('Tiky_Demce','torture'), 0);
	...
	override function update(elapsed:Float)
	{
		...
		{
			once = true;
			var snd:FlxSound = new FlxSound().loadEmbedded(Paths.sound('complete','torture'));
			snd.play();
			loadingImage.alpha = 0;
			loadingDone.alpha = 1;
	...
	override function beatHit()
	{
		...
		{
			case 5:
				createCoolText(['Creators Of'], 185);
			// credTextShit.visible = true;
			case 6:
				addMoreText('Vs Torture Mod', 185);
			case 7:
				addMoreText('Elirix', 185);
			case 8:
				addMoreText('TeiCleM', 185);
			case 9:
				ngSpr.visible = true;
			// credTextShit.visible = false;
			...
			case 10:
				deleteCoolText();
				ngSpr.visible = false;
				createCoolText(['Original Creators'], 185);
			case 11:
				addMoreText('Of The', 185);
			case 12:
				addMoreText('Tricky', 185);
			case 13:
				addMoreText('Mod', 185);
			case 14:
