		...
		switch (daStage)
		{
			case 'school' | 'schoolEvil':
				...
				if(burning){
					...
					loadGraphic(Paths.image('NOTE_fire-pixel', "torture"), true, 21, 31);
			...
			default:
				...
				if (dodge)
		            {
			            frames = Paths.getSparrowAtlas('NOTE_dodge', "torture");
			
			            animation.addByPrefix('greenScroll', 'green dodge');
			            animation.addByPrefix('redScroll', 'red dodge');
			            animation.addByPrefix('blueScroll', 'blue dodge');
			            animation.addByPrefix('purpleScroll', 'purpule dodge');
				...
				if(burning){
					...
					{
						frames = Paths.getSparrowAtlas('fourth/mech/ALL_deathnotes', "torture");
					...
					{
						frames = Paths.getSparrowAtlas('NOTE_fire', "torture");
						if(!FlxG.save.data.downscroll){
							animation.addByPrefix('blueScroll', 'blue dodge');
							animation.addByPrefix('greenScroll', 'green dodge');
						}
						else{
							animation.addByPrefix('greenScroll', 'blue dodge');
							animation.addByPrefix('blueScroll', 'green dodge');
						}
						animation.addByPrefix('redScroll', 'red dodge');
						animation.addByPrefix('purpleScroll', 'purple dodge');
                ...
                loadGraphic(Paths.image('warningNote','torture'));
				setGraphicSize(Std.int(width * 0.7));
				updateHitbox();
				antialiasing = true;
		}
