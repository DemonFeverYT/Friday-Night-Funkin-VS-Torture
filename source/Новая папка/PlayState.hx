...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (SONG.song.toLowerCase() == 'agency')
		{
			//trace("line 538");
			defaultCamZoom = 0.6;
			curStage = 'nevada';
			...
			if (SONG.song.toLowerCase() != 'consternation')
			{
				add(bg);
			...
			engineerleft = new FlxSprite(1650, 240);
			...
			engineerleft.setGraphicSize(Std.int(engineerleft.width * 0.95));
			engineerleft.antialiasing = true;
			
			add(engineerleft);

			agentleft = new FlxSprite(1350, 220);
			...
			agentleft.setGraphicSize(Std.int(agentleft.width * 0.9));
		...
		else if (SONG.song.toLowerCase() == 'consternation')
		{
            //trace("line 538");
			defaultCamZoom = 0.55;
			curStage = 'bruh';
			...
			soldatleft.frames = Paths.getSparrowAtlas('agents/SoldatLeft','torture');
			...
			soldat = new FlxSprite(-650, 300);
			soldat.frames = Paths.getSparrowAtlas('agents/Soldat','torture');
		...
		else if (SONG.song.toLowerCase() == 'helltorture')
		{
			//trace("line 538");
			defaultCamZoom = 0.35;
			...
			var stageFront:FlxSprite = new FlxSprite(-2000, -400).loadGraphic(Paths.image('agents/island_but_red','torture'));
			...
			hank.frames = Paths.getSparrowAtlas('agents/Hank','torture');
		...
		if (dad.curCharacter == 'ducky_assets')
		{
			...
			dad.setGraphicSize(Std.int(dad.width * 1.5));
			dad.y -= 0;
			dad.x -= 90;
			gf.y += 120;
			gf.x += 350;
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
        ...
        function trickyCutscene():Void // god this function is terrible
		{
			...
			trans.setGraphicSize(Std.int(trans.width * 2.3));
						...
						if (daNote.y < -daNote.height && !FlxG.save.data.downscroll || daNote.y >= strumLine.y + 106 && FlxG.save.data.downscroll)
							{
								...
								else
								{
									...
									{
										if (!daNote.isSustainNote || curStage != 'nevadaSpook')
										{
											health -= 0.075;
										...
										else if (daNote.isSustainNote && curStage == 'nevadaSpook') // nerf long notes on helltorture cuz they're too op
										{
											interupt = true;
											health -= 0.035;
	...
	public function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			else
			{
				...
				{
					case 'consternation':
						LoadingState.loadAndSwitchState(new VideoState("assets/videos/magkillsbf.webm",new PlayState()));
					default:
	...
	override function beatHit()
	{
		...
		if (curBeat == 10 && curSong == 'consternation')
		{
